...
<Select.Root
	label="Navigation"
	class="h-screen flex flex-col items-center justify-center bg-gray-700 -m-8"
	let:open
>
	<div class="relative inline-flex">
		<div
			class="relative w-[480px] inline-flex items-center rounded-2xl border-0 bg-white shadow-[0_4px_6px_-1px_rgba(0,0,0,0.1)] ring-1 ring-gray-300 overflow-hidden"
		>
			<span
		...
		{#if open}
			<Select.ListBox
				class="max-h-[320px] absolute inset-x-0 bottom-full mb-2 p-2 bg-white shadow-[0_4px_6px_-1px_rgba(0,0,0,0.1)] ring-1 ring-gray-300 rounded-2xl overflow-x-hidden overflow-y-auto"
			>
				{#each items as item}
					<Select.Item
						id={item.name}
